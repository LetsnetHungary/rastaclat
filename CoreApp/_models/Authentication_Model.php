    ...
    class Authentication_Model extends CoreApp\Model {
        ...
        public function loginAttemptUser(CoreApp\AttemptUser $a) {
          if($this->checkIfUserLoggedIn()) {
            ...
            $stmt = $this->db->prepare("SELECT password, uniquekey, webshopdb FROM users WHERE email = :email");
            $stmt->execute(array(
              ":email" => $a->e
            ));
            if($result = $stmt->fetchAll(PDO::FETCH_ASSOC)){
              ...
              CoreApp\Session::set("webshopdb", $result[0]["webshopdb"]);
        ...
        private function allowedDevice($uk, $dk) {
          $stmt = $this->db->prepare("SELECT devicename FROM devices WHERE uniquekey = :uniquekey AND devicekey = :devicekey");
          $stmt->execute(array(
            ":uniquekey" => $uk,
            ":devicekey" => $dk
          ));
